...
class TestUpdateOrcPw(unittest.TestCase):
    ...
    def check_new_login(self):
        ...
        if dict_json["status"] == True:
            ...
            self.log.info("check_new_login重新获取orc_admin_token成功", "check_new_login", 166)
            self.restore_orc_pw()  # 调用还原密码
        else:
            self.log.error("check_new_login重新获取orc_admin_token失败", "check_new_login", 169)
    ...
    def restore_orc_pw(self):
        ...
        dict_json = json.loads(res_json)  # 把json数据转换成字典对象
        if dict_json["status"] == True:
            self.log.info("restore_orc_pw orc admin密码还原成功", "restore_orc_pw", 183)
            self.restore_get_pw()
        else:
            self.log.error("restore_orc_pw orc admin密码还原失败""restore_orc_pw", 186)
