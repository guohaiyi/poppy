...
class LoginTest(unittest.TestCase):
    def setUp(self):
        ...
        self.row = [30, 31, 32, 33, 34, 35]
        self.log.info(message="----------测试开始----------", name="test06_CreateTenantAdmin.py")

    def tearDown(self):
        self.log.info(message="----------测试结束----------", name="test06_CreateTenantAdmin.py")
    ...
    def test01_cre_tenant(self):
        ...
        self.assertEqual(status_code, 200, msg=">>>test01_cre_tenant接口请求失败")
        self.assertTrue(dict_json["status"], msg=">>>test01_cre_tenant断言失败，实际返回结果：%s" % dict_json)
        self.assertEqual(dict_json["username"], "auto_user",
                         msg=">>>test01_cre_tenant断言失败，实际返回值是：%s" % dict_json["username"])
