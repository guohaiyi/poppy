...
class CreateTenantDbTest(unittest.TestCase):
    def setUp(self):
        ...
        self.row = [26, 27, 28, 29]
        self.log.info(message="----------测试开始----------", name="test05_DisEnaTenantDB.py")

    def tearDown(self):
        self.log.info(message="----------测试结束----------", name="test05_DisEnaTenantDB.py")

    def test01_DisTenDb(self):
        """禁用TenantDB"""
        self.log.info(message="test01_DisTenDb", name="test05_DisEnaTenantDB.py", line=32)
        # 设置请求数据
        ...
        if dict_json["status"] == True:
            in_status = self.check_is_success()
            if in_status == False:
                is_success = True
            else:
                is_success = False
            self.assertTrue(is_success, msg='>>>禁用DB验证不通过')

    def test02_DisTenDbFail(self):
        """禁用TenantDB失败：不存在"""
        self.log.info(message="test02_DisTenDbFail", name="test05_DisEnaTenantDB.py", line=32)
        # 设置请求数据
    ...
    def test03_EnaTenDb(self):
        """启用TenantDB"""
        self.log.info(message="test03_EnaTenDb", name="test05_DisEnaTenantDB.py", line=32)
        # 设置请求数据
        ...
        if dict_json["status"] == True:
            ...
            if in_status == False:
                ...
                is_success = False
            self.assertFalse(is_success, msg='>>>启用DB验证不通过')

    def test04_TenDbFail(self):
        """启用TenantDB成功：不存在"""
        self.log.info(message="test04_TenDbFail", name="test05_DisEnaTenantDB.py", line=32)
        # 设置请求数据
        method = self.data.get_method(self.sheet, self.row[1])
        data = self.data.get_param(self.sheet, self.row[1])
        url = self.config.get_base_url() + self.data.get_url(self.sheet, self.row[1]) + data
